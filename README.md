BLUE TICKER は、EDINET APIを最大限に活用した、**MCP (Model Context Protocol) 対応** の日本株分析 Python CLI ツールです。
...
- **あらゆる AI クライアントに対応**: Claude Desktop, Goose, LM Studio, VS Code (Cline/Roo Code) など、お好みの環境に BLUE TICKER の分析能力を統合。
- **フルスタック MCP 機能**: 銘柄検索、財務分析、有報データ抽出、ウォッチリスト・ポートフォリオ管理など **全10ツール** を提供。AI がツールを組み合わせて分析を実行します。
...
```bash
# Claude Desktop の場合
ticker mcp install-claude

# LM Studio の場合
ticker mcp install-lm-studio
```
