...
def _extract_usgaap_gp_from_html(xbrl_dir: Path) -> Optional[dict]:
    ...
    for table in soup.find_all("table"):
        ...
        for row in rows:
            ...
            numerics = [
                (i, _parse_number(c.get_text(strip=True)))
                for i, c in enumerate(cells)
                if i > 0 and _parse_number(c.get_text(strip=True)) is not None
            ]
            if len(numerics) < 2:
                continue
