...
def cmd_sector(args):
    ...
    if not sector_query:
        ...
        if fmt == "json":
            ...
            return
        print(f"\n東証33業種一覧 ({len(sectors)}業種):")
        print("-" * 50)
        print(f"{'コード':<8} {'業種名':<20} {'銘柄数':>6}")
        print("-" * 50)
        for s in sectors:
            print(f"{s['code']:<8} {s['name']:<20} {s['count']:>6}件")
        print("-" * 50)
        return

    results = master_data_manager.search_by_sector(sector_query)
    if not results:
        print(f"'{sector_query}' に一致する業種の銘柄は見つかりませんでした。")
        return
    ...
    print(f"\n[{sector_label}] の銘柄一覧 ({len(results)}件):")
    print("-" * 55)
    print(f"{'コード':<8} {'銘柄名':<25} {'市場'}")
    print("-" * 55)
    for item in results:
        print(f"{item['code']:<8} {item['name']:<25} {item['market']}")
    print("-" * 55)
