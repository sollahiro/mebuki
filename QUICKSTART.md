...
### macOSでのパスワード入力が頻繁に求められる
アプリ起動時や設定保存時に、「mebuki」や「mebuki-backend」がキーチェーンへのアクセス許可（パスワード入力）を求め、ログインパスワードの入力を要求されることがあります。

**原因**:
1. **複数プロセスによるアクセス**: Electron（メインアプリ）とPython（バックエンド）の両方がセキュリティ保護されたキーチェーンを利用するため、それぞれで許可が必要です。
2. **セキュリティ仕様**: macOSのセキュリティ機能により、未署名のバイナリや、開発・更新によって内容が変更されたバイナリがキーチェーンにアクセスする際、都度確認が求められます。

**解決方法**:
- プロンプトが表示された際に **「常に許可 (Always Allow)」** を選択してください。これにより、そのバイナリが変更されない限り、次回以降の入力を省略できます。
- コードの修正やアプリの更新を行った後は、再度入力を求められることがありますが、これは正常な動作です。
