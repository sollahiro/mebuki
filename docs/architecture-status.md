...
| EDINETキャッシュ境界 | 一部対応 | `EdinetCacheStore` + TTL + XBRL容量上限・LRU対応済み。CacheManager統合ポリシーは未対応 |
| EDINET補完パイプライン | 対応済み | 共通フォーマット化・H2計算整理まで完了 |
| 個別分析キャッシュ粒度 | 対応済み | XBRL parse・書類リスト・J-QUANTS を独立キャッシュ化。年度別粒度化は恩恵薄として見送り |
...
- 最終metricsの年度別粒度化: 指標別バージョン管理が必要で実装複雑度が高い。`_apply_*` 再計算は純 CPU で実際のコストは小さく、XBRL パース・書類リスト・J-QUANTS はすでに独立キャッシュ済みのため恩恵が薄いと判断し見送り。
...
4. ~~pyrightをCIに組み込む判断（#9 残り）~~ → 完了（`ci.yml` に `pyright mebuki/` を追加、`pyrightconfig.json` で basic モード全体適用）
