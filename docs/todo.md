...
- EDINET API 呼び出しが逐次実行になりやすく、銘柄横断ではキャッシュとレート制御の設計が重要
...
- 値型が `str | float | None` 程度に絞れる場合でも `Any` で代替している箇所がある
  - ポートフォリオ系、銘柄マスタ/セクター系、XBRL抽出結果の主要戻り値は TypedDict 化済（2026-05-01）
...
直近報告書選定は `EdinetFetcher` に移し、`EdinetAPIClient` は日次書類取得・XBRLダウンロードに集中。
...
デッドコード（`extract_quarterly_data`・`_calculate_quarter_end_date` など）を削除。
