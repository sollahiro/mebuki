...
- `mebuki analyze` などの各サブコマンドに backend 選択オプションを増やさない。
...
```bash
mebuki config set edinet-backend local
mebuki config set edinet-key <EDINET_API_KEY>

mebuki config set edinet-backend remote
mebuki config login
```
...
- remote backend利用時の `mebuki cache status` 表示内容
