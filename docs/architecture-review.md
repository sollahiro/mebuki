...
| レイヤー | 主なファイル | 現在の責務 |
|---|---|---|
| CLI | `blue_ticker/app/cli/*` | 入力検証、出力形式、DataService呼び出し |
...
| 出力整形 | `utils/output_serializer.py` | CLI標準JSONからデバッグフィールドを除外 |
...
内部の `CalculatedData` には計算追跡用のフィールドを残す。一方、CLIの標準JSONでは `utils/output_serializer.py` を通して以下を除外する。
...
`ticker analyze --include-debug-fields` を指定した場合だけ、上記を含む。`SalesLabel` / `OPLabel` は「売上高ではなく純収益」「営業利益ではなく事業利益/経常利益」といった意味情報なので、標準JSONにも残す。
...
ローカルMCPサーバーと `ticker mcp` は廃止済み。AIエージェントからのローカル操作はCLI + Skillsで行う。

| 機能 | CLI | メモ |
|---|---|---|
| 銘柄検索 | `search` | 社名・コード検索 |
| 財務分析 | `analyze` | 年次財務分析 |
| 半期 | `analyze --half` | H1/H2 半期推移 |
| デバッグフィールド | `analyze --include-debug-fields` | 明示指定時のみ内部メタデータを出力 |
| EDINET一覧 | `filings` | 有価証券報告書等の一覧 |
| EDINET本文 | `filing` | 指定セクション抽出 |
| セクター | `sector` | 東証33業種検索 |
| watch/portfolio | `watch`, `portfolio` | ウォッチリスト・ポートフォリオ管理 |
| キャッシュ可視化 | `cache status` | キャッシュ状態確認 |
| キャッシュ削除 | `cache clean` | dry-runがデフォルト |

運用方針:

- CLIの標準分析JSONは serializer 経由でデバッグフィールドを除外する。
...
3. `CalculatedData` の公開キー削除  
   CLI利用者への互換性影響が大きい。renameよりalias期間を置くべき。
...
- `cache clean` はdry-runをデフォルトにしてCLIで提供
- docsにキャッシュ方針を明記済み

### Phase 2: 指標の出所整理

- 指標ごとの source/method/docID/unit/label を `MetricSources` に追加済み
- CLI標準JSONでは `MetricSources` / `IBDComponents` / `GrossProfitMethod` / `IBDAccountingStandard` を除外済み
- `--include-debug-fields` でデバッグフィールドを明示的に出力可能
