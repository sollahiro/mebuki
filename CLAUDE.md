...
- **レビュー用セッションは `read-only`、実装用セッションは書き込み権限ありで新規に開始する。** `mcp__codex__codex-reply` にはサンドボックス設定がないため、`read-only` セッションへの返信では書き込みができない。承認後は `mcp__codex__codex`（新規セッション）で `sandbox=workspace-write` を指定して実装を依頼する。外部通信（`git push`・新規パッケージの `pip install` 等）が必要な場合のみ `danger-full-access` を使う
