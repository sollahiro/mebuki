...
- **レビュー用セッションは `read-only`、実装用セッションは必ず `danger-full-access` で新規に開始する。** `mcp__codex__codex-reply` にはサンドボックス設定がないため、`read-only` セッションへの返信では書き込みができない。承認後は `mcp__codex__codex`（新規セッション）で `sandbox=danger-full-access` を指定して実装を依頼する
