...
`TypedDict` や戻り値型を追加・変更すると、`Any` / `dict` に隠れていた曖昧さが表面化する。
型定義を触った場合は、変更対象モジュールに対して pyright / basedpyright を実行し、
新しい型エラーを残したまま完了扱いにしない。

いきなり全リポジトリを strict 化しない。まずは変更対象モジュール単位で警告ゼロを維持し、
必要に応じて対象範囲を広げる。

型エラーを `cast` や `Any` で消すのは外部ライブラリ境界・動的 JSON 境界に限定する。
ドメイン内部では TypedDict / Mapping / Sequence / `X | None` ガードで表現する。
