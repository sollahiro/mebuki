...
ユーザーが可視化や資料化を求めた場合は、以下のグラフを基本形にする。グラフは見栄えよりも、年次推移・構成変化・前年差要因が読み取れることを優先する。
...
| キャッシュフロー | 営業CF・投資CF・フリーCFの時系列棒グラフ | 年度ごとに営業CF、投資CF、フリーCFを横並びにし、ゼロラインを明示する。投資CFとFCFの符号が一目で分かるようにする |
...
キャッシュフローのグラフでは、営業CFが安定しているか、投資CFの負担が大きすぎないか、フリーCFが継続的にプラスかを確認できるようにする。
