...
description: BLUE TICKER CLIを使って日本株の検索・財務分析・有価証券報告書抽出を行う汎用ワークフロースキル
---
...
`ticker analyze` で取得した貸借対照表データを、各年度につき「資産」と「負債・純資産」の2本の積み上げ棒グラフとして可視化し、資産構成と資本調達構造の変化を年次で比較する。

- **左棒（資産サイド）**: 固定資産（下）＋ 流動資産（上）
- **右棒（負債・純資産サイド）**: 純資産（下）＋ 固定負債（中）＋ 流動負債（上）
...
- **見せ方**: 資産サイドと調達サイドを必ず同じ縦軸スケールで表示し、総資産の拡大・縮小と構成比の変化を同時に読めるようにする
...
## 営業利益増減分析（連結ウォーターフォール）

### 概要

営業利益の前年差を「売上差影響（数量の影響）」「粗利率差影響（採算の影響）」「販管費影響（コストの影響）」の3要素に分解し、前年差の橋渡しをウォーターフォールで示す。交互作用を排除した設計により、3要素の合計が営業利益の前年差額と完全に一致する。

5年推移を見せる場合は、期末営業利益の棒を年度ごとに置き、各年度間を「売上差影響」「粗利率差影響」「販管費影響」の3本で接続する。単年の前年差だけでなく、どの要因が毎年繰り返し効いているかを読むことを重視する。
...
| `SGAChangeImpact` | 販管費影響 | 百万円 | コスト効果（増加はマイナス） |
...
年次テーブルでは `SGAChangeImpact` 相当の行を使う。

分析・可視化時の表示名は、費用増だけでなく費用減のプラス効果も自然に読めるよう **「販管費影響」** に統一する。
...
**推奨チャート**: ウォーターフォールチャート。期末営業利益を濃色の基準棒、売上差影響を青、粗利率差影響を緑、販管費影響を赤系で表現し、前年差要因を1本の流れとして接続する。
...
| 販管費影響が大きくマイナス | 積極的な先行投資か、コスト管理の問題かを定性情報で確認 |

> **データ欠損時**: 売上総利益または販管費がXBRLから取得できない年は `None` となりすべての分解値が非表示になる。その場合は `ticker filing` で有報のMD&Aセクションを参照する。

---

## キャッシュフロー構造分析（営業CF・投資CF・FCF）

### 概要

`ticker analyze` で取得したキャッシュフローを、営業CF・投資CF・フリーCFの3系列で重ねて可視化する。利益ではなく実際の資金創出力と投資負担を確認し、営業利益や当期純利益とのズレを読む。

- **営業CF**: 本業から生む現金。継続的にプラスで、営業利益と大きく乖離しないかを見る
- **投資CF**: 設備投資・M&A等の資金流出入。通常はマイナス方向に出る
- **フリーCF**: 営業CF + 投資CF。株主還元・借入返済・成長投資の原資

### 使用フィールド（CalculatedData）

| フィールド | 表示ラベル | 単位 | 備考 |
|---|---|---|---|
| `CFO` | 営業CF | 百万円 | 営業活動によるキャッシュフロー |
| `CFI` | 投資CF | 百万円 | 投資活動によるキャッシュフロー |
| `CFC` / `FreeCF` | フリーCF | 百万円 | 営業CF + 投資CF |

### グラフ仕様

- **横軸**: 年度または四半期・半期。年次の構造確認では5年から10年、季節性確認では `--half` を使う
- **縦軸**: 百万円。ゼロラインを必ず表示し、プラス・マイナスの符号を読みやすくする
- **棒の配置**: 営業CF・投資CF・フリーCFを同じ時点で横並びまたは重ね表示する
- **FCFの幅**: フリーCFは営業CF・投資CFより少し細く表示し、合成結果であることを視覚的に区別する
- **色**: 営業CFは明るいグレー、投資CFは濃いグレー、FCFは黄色系など、3系列が一目で分かる配色にする

### 読み方・着眼点

| パターン | 意味 |
|---|---|
| 営業CFが安定プラス / FCFもプラス | 事業の現金創出力が強く、投資後も余剰資金が残る |
| 営業CFはプラス / 投資CFが大きくマイナス / FCFが一時マイナス | 成長投資・設備投資局面。将来の売上・粗利率改善とセットで確認 |
| 営業利益は黒字 / 営業CFが弱い | 売掛金・棚卸資産・一過性要因で利益の質に注意 |
| 投資CFが継続プラス | 資産売却や投資回収が続いている可能性。成長投資の不足も確認 |
| FCFが継続マイナス | 借入・増資・資産売却で資金を補っていないかBSと合わせて確認 |

---

## 利益の現金化分析（FCFウォーターフォール）

### 概要

営業利益増減分析が「利益を作る力」を見るのに対して、利益の現金化分析は「利益を現金として残す力」を見る。営業利益から出発し、非現金費用である減価償却費、営業CFとの差分、投資CFを橋渡ししてフリーCFに接続する。
...
検証:
営業利益 + 減価償却費 + その他現金化差分 = 営業CF
営業CF + 投資CF = FCF
```

- **営業利益**: 会計上の本業利益
- **減価償却費**: CF計算書の営業CF調整項目から抽出する非現金費用の戻し入れ
- **その他現金化差分**: 運転資本、税金、利息、引当金、その他調整項目をまとめた差分。運転資本を無理に単独抽出せず、営業CFとの整合を優先する
- **投資CF**: 設備投資・M&A・資産売却など投資活動による資金増減
- **FCF**: 投資後に残った自由キャッシュ
...
|---|---|---|---|
| `OP` | 営業利益 | 百万円 | 既存の本業利益 |
| `DepreciationAmortization` | 減価償却費 | 百万円 | CF計算書から抽出 |
| `OtherCashConversionGap` | その他現金化差分 | 百万円 | `CFO - OP - DepreciationAmortization` |
| `CFI` | 投資CF | 百万円 | 投資活動によるキャッシュフロー |
| `CFC` / `FreeCF` | フリーCF | 百万円 | `CFO + CFI` |

### 可視化（ウォーターフォールチャート）

営業利益からFCFまでを以下の順番で接続する。
...
減価償却費は通常プラス寄与、投資CFは設備投資局面では通常マイナス寄与として表示する。その他現金化差分が大きくマイナスの場合は、運転資本増加・税金支払い・利息支払い・一過性調整などを疑い、必要に応じて有報MD&Aを確認する。
